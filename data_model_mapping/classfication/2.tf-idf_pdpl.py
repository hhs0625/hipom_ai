...
data_model_master_file_path = 'master_model/data_model_master_export_rdoc.csv'
...
test_csv['c_thing_pdpl'] = ''
test_csv['c_property_pdpl'] = ''
test_csv['c_score_pdpl'] = ''
...
for index, test_row in tqdm(test_csv.iterrows(), total=test_csv.shape[0], desc="Processing"):
    ...
    test_csv.at[index, 'pdpl_c_thing'] = best_match_row['thing']
    test_csv.at[index, 'pdpl_c_property'] = best_match_row['property']
    test_csv.at[index, 'pdpl_c_score'] = best_match_score

# Add new fields for correctness checks
test_csv['pdpl_thing_correct'] = test_csv['pdpl_c_thing'] == test_csv['thing_pattern']
test_csv['pdpl_property_correct'] = test_csv['pdpl_c_property'] == test_csv['property_pattern']
test_csv['pdpl_tp_correct'] = test_csv['pdpl_thing_correct'] & test_csv['pdpl_property_correct']

# Calculate the percentage of correct items where p_MDM is True
mdm_true = test_csv[test_csv['MDM']]  # Ensure p_MDM is considered as a string
correct_mdm_true = mdm_true[mdm_true['pdpl_tp_correct']]

# Calculate percentage
percentage_correct = (len(correct_mdm_true) / len(mdm_true)) * 100 if len(mdm_true) > 0 else 0

print(f"PDPL accuracy: {percentage_correct:.2f}%")
